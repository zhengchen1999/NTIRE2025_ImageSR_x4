...
class OmniShift(nn.Module):
    ...
    def forward_train(self, x):
        ...
        out = self.alpha[0]*out1x1 + self.alpha[1]*out3x3 +self.alpha[2]*out5x5
        return out


    def forward(self, x,x_size):

        x = rearrange(x,'b (h w) c-> b c h w',h=x_size[0]).contiguous()

        return rearrange(self.forward_train(x),'b c h w->b (h w) c').contiguous()
...
class ChannelAttention(nn.Module):
    ...
    def __init__(self, num_feat, squeeze_factor=10):
        super().__init__()
...
class CAB(nn.Module):
    def __init__(self, num_feat, compress_ratio=8, squeeze_factor=24):
        super().__init__()
...
class Mamba(nn.Module):
    def __init__(
        ...
        # print('d_model d_innrer: ', self.d_model,d_in_proj)
        self.in_proj = nn.Linear(self.d_model, d_in_proj, bias=bias, **factory_kwargs)

        conv_dim = self.d_inner + 2 * (2 * self.ngroups * self.d_state)
        self.conv_mixer = OmniShift(d_model)
        self.conv = nn.Conv2d(conv_dim,conv_dim, 3,1,1,groups=conv_dim)
        ...
        # self.norm1 = RMSNormGated(self.d_inner, eps=1e-5, norm_before_gate=True, **factory_kwargs)

        self.out_proj = nn.Linear(self.d_inner, self.d_model, bias=bias, **factory_kwargs)

        # self.alpha = nn.Parameter(torch.randn(self.d_inner), requires_grad=True)
        ...
        self.mu = nn.Parameter(1.0 - torch.pow(ddd, ratio_1_to_almost0),
                               requires_grad=True)


    def forward(self, u, x_size):
        """
        u: (B, L, D)
        Returns: same shape as u
        """
        batch, seqlen, dim = u.shape
        u = self.conv_mixer(u,x_size)*self.mu + (1-self.mu)*u
        ...
        dt_wh = dt.reshape(batch, x_size[0], x_size[1], -1).transpose(1,2)
        dt_wh = rearrange(dt_wh,'b w h c->b (w h) c')
        dt = torch.cat((dt[:, :, :self.nheads], torch.flip(dt[:, :, self.nheads:], (1,)),
                        dt_wh[:, :, :self.nheads], torch.flip(dt_wh[:, :, self.nheads:], (1,))), dim=0)
        dt = F.softplus(dt + self.dt_bias)  # (2 * B, L, nheads)
        assert self.activation in ["silu", "swish"]

        # 2D Convolution
        xBC = self.act(
            self.conv(rearrange(xBC,'b (h w) c->b c h w',h=x_size[0]).contiguous())
        )  # (B, L, self.d_inner + 2 * (2 * ngroups * d_state))
        xBC = rearrange(xBC,'b c h w-> b (h w) c').contiguous()

        # Split into 3 main branches: X, B, C
        # These correspond to V, K, Q respectively in the SSM/attention duality
        x, BC = torch.split(xBC, [self.d_inner, 2 * (2 * self.ngroups * self.d_state)], dim=-1)
        # x_og = x
        x_wh = x.reshape(-1, x_size[0], x_size[1], self.d_inner).transpose(1,2)
        x_wh = rearrange(x_wh,'b w h c->b (w h) c')
        x = torch.cat((x, torch.flip(x, (1,)),x_wh, torch.flip(x_wh,(1,))), dim=0)
        
        BC_wh = BC.reshape(batch, x_size[0], x_size[1], -1).transpose(1,2)
        BC_wh = rearrange(BC_wh,'b w h c->b (w h) c')
        BC = torch.cat(
            (BC[:, :, :2 * self.ngroups * self.d_state],
             torch.flip(BC[:, :, 2 * self.ngroups * self.d_state:], (1,)),
             BC_wh[:, :, :2 * self.ngroups * self.d_state],
             torch.flip(BC_wh[:, :, 2 * self.ngroups * self.d_state:], (1,))),
            dim=0
        )
        ...
        y1, y2 = y[:batch], torch.flip(y[batch:2*batch], (1,))
        y3, y4 = y[2*batch:3*batch], torch.flip(y[3*batch:], (1,))
        y3 = rearrange(y3, 'b (w h) c->b (h w) c', h=x_size[0])
        y4 = rearrange(y4, 'b (w h) c->b (h w) c', h=x_size[0])
        y=y1+y2+y3+y4
...
class ATDTransformerLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.mamba_attn = Mamba(dim,36, layer_idx=idx, headdim=30, expand=2)

        self.convffn = FFN5(dim, mlp_ratio)
    ...
    def forward(self, x, td, x_size, params):
        ...
        shortcut = x
        x = self.norm1(x)
        x_mamba = self.mamba_attn(x,x_size)
        qkv = self.wqkv(x)
        ...
        if self.shift_size > 0:
            ...
            attn_x = shifted_x
        x_win = attn_x
        x = shortcut + x_win.view(b, n, c) + x_atd + x_aca + x_mamba
