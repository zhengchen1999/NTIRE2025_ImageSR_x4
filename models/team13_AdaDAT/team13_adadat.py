...
class AdaDAT(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(self.num_layers):
            ...
            layer_ada = ChannelAttention(embed_dim)
            self.layers_ada.append(layer_ada)
    ...
    def forward_features(self, x):
        ...
        x_size = [H, W]
        x = self.before_RG(x)
        for idx, (layer_ada, layer) in enumerate(zip(self.layers_ada, self.layers)):
            x = layer(x, x_size) + rearrange(layer_ada(rearrange(x, "b (h w) c -> b c h w", h=H, w=W).contiguous()), "b c h w ->b (h w) c").contiguous()
